...
Четыре вкладки поверх API выше — статистика обучения **в прямом эфире** (SSE-поток `/training/history/stream`, графики loss/accuracy/F1 и индикатор `running`/`completed` обновляются сами по мере обучения, без перезагрузки страницы), ручной тест прогноза на точке (форма с готовыми пресетами сценариев + вклад модальностей по SHAP + сравнение прогнозов), внесение удобрений (редактируемая таблица участков → оптимальные дозы → скачивание карты-задания в ISO 11783-10, L5+L7 живьём), каталог датасетов (разворачиваемая карточка — источник/состав/лицензия/ограничения — + наличие на диске + загрузка нового датасета архивом).
...
**`build/` отслеживается в git и содержит результат именно такого smoke-test прогона** (`build/leaf_classifier/best_model.pt`, `build/ner_model/model.safetensors` + `history.json` для обоих) — веса на процедурно сгенерированных данных, а не на реальных фотографиях поражений или агрономическом тексте, предназначены для проверки, что оба пайплайна воспроизводимо сходятся, а не как готовые к продовому использованию модели. При реальном обучении (раздел выше) `git add build/...` заменит их результатом на настоящих данных. Два подкаталога `build/` в git не попадают: `build/*/checkpoint-*/` (промежуточные resume-чекпоинты Trainer — дублируют состояние оптимизатора, не нужны после завершения обучения) и `build/models/` (demo-модель L4, обучаемая на лету на синтетике при первом обращении к `/prediction/predict` — коммит этого файла свёл бы на нет стартовую проверку production-конфигурации, см. «Развёртывание (production)» ниже: она как раз должна обнаруживать отсутствие настоящей модели, а не молча считать synthetic-файл настоящим).
