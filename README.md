...
Три вкладки поверх API выше — статистика обучения (графики loss/accuracy/F1 по эпохам и гиперпараметры из `build/*/history.json`), ручной тест прогноза на точке (форма с готовыми пресетами сценариев + вклад модальностей по SHAP + сравнение прогнозов), каталог датасетов (разворачиваемая карточка — источник/состав/лицензия/ограничения — + наличие на диске + загрузка нового датасета архивом).
