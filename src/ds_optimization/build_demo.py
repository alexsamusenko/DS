...
def run_demo():
    ...
    print("Дифференцированное внесение против равномерного")
    ...
    assert profit_diff >= profit_uniform - 1e-6, "Дифференцированное внесение не должно уступать равномерному при том же бюджете"
...
def run_variability(seeds=tuple(range(1, 11))):
    ...
    единственный расчётный пример (seed=5, используемый в run_demo)
    ...
    assert (gains >= -1e-6).all(), "Дифференцированное внесение не должно уступать равномерному ни на одной из конфигураций"

    return gains


def run_cost(n_calls=50):
    """Измерить вычислительную стоимость оптимизации уровня L5: время
    формирования карты-задания для одного поля на процессоре, без
    оптимизации инференса -- нужно для оценки практического эффекта
    (сокращения времени цикла принятия решения).
    """
    plots = generate_plots()
    total_area = float(plots["area"].sum())
    budget = 70.0 * total_area
...
def run_sensitivity_dose_level(avg_doses=(30, 50, 70, 90, 110, 130)):
    ...
    к среднему уровню бюджета -- run_demo() и run_variability()
    ...
    print(f"\nЧувствительность выигрыша к среднему уровню бюджета")
    ...
    assert all(r["profit_diff"] >= r["profit_uniform"] - 1e-6 for r in rows), (
        "Дифференцированное внесение не должно уступать равномерному ни при одном уровне бюджета"
    )
