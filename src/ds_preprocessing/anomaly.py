"""Детекция аномалий по модифицированной z-оценке (MAD)."""
...
def detect_anomalies(X, mask_observed, threshold=DEFAULT_THRESHOLD):
    ...
    Ряды агрономических показателей, как правило, нестационарны (несут
    временной тренд). Поэтому MAD считается не по сырым
